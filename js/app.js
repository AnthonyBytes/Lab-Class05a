...
function sumArray(sumArr) { //eslint-disable-line
  ...
  for (let i = 0; i < sumArr.length; i++) {
    ...
    numbersString += `${sumArr[i]}`;
    if (i < sumArr.length - 1) {
      numbersString += ',';
    }
  }

  return [sumResult, `${numbersString} was passed in as an array of numbers, and ${sumResult} is their sum.`];
}
...
function multiplyArray(multArr) { //eslint-disable-line
  ...
  for (let i = 0; i < multArr.length; i++) {
    ...
    numbersString += `${multArr[i]}`;
    if (i < multArr.length - 1) {
      numbersString += ',';
    }
  }

  return [productResult, `The numbers ${numbersString} have a product of ${productResult}.`];
}
